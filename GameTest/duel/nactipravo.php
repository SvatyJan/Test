...
// L1 STATS
				...
				$dotaz1 = "SELECT * FROM game_stats WHERE username='$player2' LIMIT 1";
				...
				if (mysqli_num_rows($result1) > 0) {
					while ($row = mysqli_fetch_assoc($result1)) {						
						...
						// L1 ITEMS
						$dotaz2 = "SELECT * FROM game_items WHERE username='$player2' AND equiped=1";
						...
						if (mysqli_num_rows($resulticon) > 0) {
							while ($rowicon = mysqli_fetch_assoc($resulticon)) {
								...
								array_push($loadR1, $name, $playericon, $hp, $armor, $MR, $dmg, $str, $agi, $intel);
								
							}
						}
						// <L1 ICON>
						
						array_push($loadR1, @$weapon);					
						//echo "<br>";
				...
				// L1 SPELLS
				$dotaz3 = "SELECT * FROM game_spellbar WHERE username='$player2' LIMIT 1";
				...
				if (mysqli_num_rows($result3) > 0) {
					while ($row3 = mysqli_fetch_assoc($result3)) {
						...
						array_push($loadR1, $passiveSpell, $firstSpell , $secondSpell, $thirdSpell, $ultimateSpell, $icon1 , $icon2, $icon3, $icon4, $icon5, $playerlevel);
